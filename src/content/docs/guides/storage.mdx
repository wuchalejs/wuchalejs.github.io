...
The storage interface allows for composing storages for different purposes.
These are the built-in ones that can be imported from `wuchale`. They conform
to the interface while being based on other storages.

### `migrateStorage`

This can be temporarily used to migrate between drivers. The way to migrate is:
...
This can be used to separate items by type (`message`, `url`) into different
storages. For example, to store URL translations in the `json` storage (which
stores all locales in a single file), and normal messages in `pofile`:
...
This separates the messages by locale. For example, `en` in `pofile` and `es` and `de` in `json`:

```js
  storage: storageByLocale([
    [['en'], pofile()],
    [['es', 'de'], json()],
  ]),
```
