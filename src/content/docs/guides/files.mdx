---
title: File Structure
description: Understand wuchale's file structure - discover how .po catalogs, compiled .compiled.js files, and loader/proxy scripts work together for efficient internationalization.
---

Unless configured to write other files to disk, `wuchale` only writes files
that you should commit.

import { FileTree } from '@astrojs/starlight/components';

<FileTree>
- vite.config.js
- src
    - locales
        - en.po Source catalog
        - es.po Translation catalog
        - loader.js Loader file for the adapter
        - App.svelte Your components
</FileTree>

If you configure the loader to write the compiled catalogs and the proxy,

<FileTree>
- vite.config.js
- src
    - locales
        - en.po           Source catalog
        - en.compiled.js  Source catalog compiled (if [writeFiles.compiled](/reference/adapter-common/#writefilescompiled))
        - es.po           Translation catalog
        - es.compiled.js  Translation catalog compiled
        - proxy.js        Proxy file for catalogs (if [writeFiles.proxy](/reference/adapter-common/#writefilesproxy))
        - loader.js       Loader file for the adapter
    - App.svelte          Your components
</FileTree>

If you configure it to write the transformed code as well, it will write it to
`src/locales/.output/` or the `outDir` you configure.
