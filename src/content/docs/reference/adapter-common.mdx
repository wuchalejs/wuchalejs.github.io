...
Where
- `localesDir` is [this](/reference/config/#localesdir)
...
:::note
The patterns should be relative to the wuchale config file, not absolute paths.
wuchale always checks them against relative paths.
:::
...
Storage handler. See the [Storage drivers](/guides/storage/) and the [Storage
...
The default value is a pattern to support [built-in pluralization](/guides/plurals-l10n/#built-in-pluralization).

## `heuristic`

**type**: `(msg: Text, file: string) => 'message' | 'url' | false | null | undefined`
**default**: (depends on adapter)
...
If it returns `null` or `undefined`, the default heuristic will be used. Here is the full definition of the `Text` type

<ShowType file="text.ts" name="Text" />

The `Text.path` property is very useful because it gives very useful
information about the location of the text. It is from top to bottom. For
example, if there is a text with a body of `Hello` and this `path`:

```js
[
    {type: 'element', name: 'p'},
    {type: 'expression'},
    {type: 'call', kind: 'tagged', name: 'foo'},
]
```

Then the heuristic can deduce that it's something like:
...
<ShowType file="adapters.ts" name="LoadGroupPatt" />

This applies only when `loading.granular` is enabled. It should be an array of
glob pattern(s) for each desired group. Any file that belongs to the adapter
but is not matched by any group will have its own "one-member group".
