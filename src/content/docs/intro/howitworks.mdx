...
Messages are extracted into the [catalogs](/intro/concepts#catalogs). From the above example, this is extracted:
...
translations from the [compiled catalogs](/intro/concepts#compiled-catalogs).
The example is transformed into:

```svelte
<script>
    import { getRuntime as _w_load_rx_ } from '../locales/main.loader.svelte.js'
...
exchanging the catalogs with a translator.
...
that the indices of the messages inside the compiled catalogs match those inside
