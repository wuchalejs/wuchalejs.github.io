...
4. **Initialize locales**
    ...
    - If the loader didn't exist before, it scans the source files and extracts the messages as a first time scan
...
<Tabs>
    ...
    <TabItem label="Without Vite">
        ...
        1. You write your code
        1. You invoke the CLI command `wuchale extract` to extract the messages into the catalog files

            During this step, if configured, [Gemini](/guides/gemini) does the automatic translation

        1. The translator translates the messages by editing the extracted `.po` files
        1. You invoke the CLI command `wuchale extract` again to compile the translated messages and write them to disk
