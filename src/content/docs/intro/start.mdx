...
Inside of these, which messages exactly are extracted is decided by the adapter in
use, because not all text should be extracted as a message. Some
...
function receives the message along with other details and can decide whether
to extract it or not.
