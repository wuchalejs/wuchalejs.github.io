...
export const handle = async ({ event, resolve }) => {
    // Extract locale from query parameter, defaulting to 'en'
    const locale = event.url.searchParams.get('locale') ?? 'en'
    
    // Note: For static sites using @sveltejs/adapter-static with a dynamic [lang] route:
...
layout load functions, you have to await the parent one to make sure
that the nested one runs after the catalogs are loaded.

```js
// src/routes/nested/+layout.js

/** @type {import('./$types').LayoutLoad} */
export const load = async ({ parent }) => {
    await parent()
    return {
        nestedMsg: 'Hello from nested layout'
    }
}
```
:::
...
- If the value is inside `goto()` it is extracted as a URL (can be disabled by using the heuristic `svelteDefaultHeuristic` from this package).
