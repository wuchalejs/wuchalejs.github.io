...
#### `+page.js` and similar files

The Svelte adapter only handles `.svelte` files and `.svelte.{js,ts}` files.
However, you may have translatable strings in `+page.{js,ts}`,
`+layout.{js,ts}`, etc. If so, you can use the Vanilla adapter for those files
...
[Learn more about using multiple adapters](/guides/multiple-adapters/).
